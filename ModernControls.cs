...
internal sealed class ToggleSwitch : CheckBox, IThemeAware
{
    ...
    public bool FloatOnBackground { get; set; }

    public void ApplyPalette(AppPalette palette)
    {
        _palette = palette;
        BackColor = FloatOnBackground ? Color.Transparent : ResolveHostBackColor();
        ForeColor = palette.Text;
        Invalidate();
    }
...
internal sealed class FloatingSwitchRow : Control, IThemeAware
{
    private AppPalette _palette = AppPalette.Dark;
    ...
    {
        e.Graphics.SmoothingMode = SmoothingMode.AntiAlias;
        using var background = new SolidBrush(ResolveHostBackColor());
        e.Graphics.FillRectangle(background, ClientRectangle);

        var track = new Rectangle(0, 7, 42, 22);
        var knobSize = 18;
        var knobX = Checked ? 21 : 3;
        var trackColor = Checked ? _palette.AccentAlt : Color.FromArgb(32, 43, 62);
        if (!Enabled)
        {
            trackColor = Color.FromArgb(54, 62, 78);
        }
        else if (_hovered)
        {
            trackColor = Checked
                ? Blend(_palette.AccentAlt, Color.White, _pressed ? 0.04f : 0.10f)
                : Color.FromArgb(40, 54, 76);
        }
        ...
        using var knobBrush = new SolidBrush(_palette.IsDark ? Color.White : Color.FromArgb(250, 252, 255));
        e.Graphics.FillEllipse(knobBrush, knobX, 9, knobSize, knobSize);

        TextRenderer.DrawText(
            e.Graphics,
            Text,
            Font,
            new Rectangle(58, 0, Width - 58, Height),
            Enabled ? _palette.Text : _palette.MutedText,
            TextFormatFlags.VerticalCenter | TextFormatFlags.Left | TextFormatFlags.EndEllipsis);
    }
